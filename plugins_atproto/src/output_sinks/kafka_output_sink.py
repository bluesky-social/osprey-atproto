    ...
    if err is not None:
        if isinstance(err, KafkaError):
            ...
            if code == "MSG_SIZE_TOO_LARGE":
                ...
                logger.error(
                    f"Failed to deliver message: {msg.topic()}/{msg.partition()}/{msg.offset()}: {err}"
                )
                worker_metrics.producer_acks.labels(status="error").inc()
        else:
            logger.error(
                f"Failed to deliver message for unknown reason, not a KafkaError: {err}"
            )
...
class KafkaOutputSink(BaseOutputSink):
    ...
    def ensure_topic(self):
        ...
        try:
            ...
            if self.output_topic not in metadata.topics:
                ...
                topic = NewTopic(
                    self.output_topic, num_partitions=3, replication_factor=3
                )
    ...
    def push(self, result: ExecutionResult) -> None:
        ...
        if "image_results" in data and data["image_results"] is not None:
            for cid in data["image_results"]:
                logger.info(f"checking flagged images for {cid}")
                image = data["image_results"][cid]

                if "flagged" in image and image["flagged"] is not None:
                    flagged = image["flagged"]

                    if not flagged["is_match"]:
                        continue

                    subject = did
                    if flagged["action_level"] == "record":
                        subject = uri

                    if flagged["action"] == "label":
                        labels.append(
                            OutputLabelEffect(
                                effect_kind=AtprotoEffectKind.ATPROTO_EFFECT_KIND_ADD,
                                subject_kind=EntityToSubjectKind(subject),
                                label=StringToAtprotoLabel(flagged["action_value"]),
                                comment=flagged["description"],
                                rules=["Flagged Image"],
                            )
                        )
                    elif flagged["action"] == "tag":
                        tags.append(
                            OutputTagEffect(
                                effect_kind=AtprotoEffectKind.ATPROTO_EFFECT_KIND_ADD,
                                subject_kind=EntityToSubjectKind(subject),
                                tag=flagged["action_value"],
                                comment=flagged["description"],
                                rules=["Flagged Image"],
                            )
                        )
                    elif flagged["action"] == "takedown":
                        takedowns.append(
                    ...
                    if flagged["action"] == "report" or flagged["always_report"]:
                        reports.append(
                            OutputReportEffect(
                                subject_kind=EntityToSubjectKind(subject),
                                report_kind=AtprotoReportKind.ATPROTO_REPORT_KIND_VIOLATION,
                                comment=flagged["description"],
                                rules=["Flagged Image"],
                            )
                        )
        ...
        for effects in result.effects.values():
            for effect in effects:
                ...
                if isinstance(effect, AtprotoLabelEffect):
                    ...
                    emails.append(
                        OutputEmailEffect(
                            email=effect.email, comment=effect.comment, rules=rule_names
                        )
                    )
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - 1:
                    ...
                    raise BufferError(
                        f"Failed to produce message after {self.max_retries} retries"
                    )
