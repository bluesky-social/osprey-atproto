...
class TimestampAge(UDFBase[TimestampAgeArguments, float]):
    ...
    def execute(self, execution_context: ExecutionContext, arguments: TimestampAgeArguments) -> float:
        try:
            timestamp = arrow.get(arguments.timestamp)
            seconds_elapsed = (arrow.now() - timestamp).total_seconds()
            return seconds_elapsed
        except Exception:
            # return a really big number for now if the account age doesn't show up
            return 999_999_999
